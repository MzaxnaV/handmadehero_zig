...
const i16x8 = h.Intrinsics.i16x8;
...
pub const playing_sound = struct {
    ...
    ID: h.FileFormats.sound_id,
    samplesPlayed: f32,
    next: ?*playing_sound,
};

pub const audio_state = struct {
    permArena: *h.Data.memory_arena,
...
fn OutputTestSineWave(gameState: *h.game_state, soundBuffer: *Platform.sound_output_buffer, toneHz: u32) void {
    ...
    while (sampleIndex < 2 * soundBuffer.sampleCount) : (sampleIndex += 2) {
        ...
        if (!IGNORE) {
            sineValue = h.Intrinsics.Sin(gameState.tSine);
...
pub fn PlaySound(audioState: *audio_state, soundID: h.FileFormats.sound_id) *playing_sound {
...
pub fn OutputPlayingSounds(audioState: *audio_state, soundBuffer: *Platform.sound_output_buffer, assets: *h.Asset.game_assets, tempArena: *h.Data.memory_arena) void {
    Debug.TIMED_FUNCTION(.{});
    // AUTOGENERATED ----------------------------------------------------------
    ...
    const mixerMemory = h.Data.BeginTemporaryMemory(tempArena);
    defer h.Data.EndTemporaryMemory(mixerMemory);

    const generationID = h.Asset.BeginGeneration(assets);
    defer h.Asset.EndGeneration(assets, generationID);
    ...
    while (playingSoundPtr.*) |playingSound| {
        ...
        while (totalChunksToMix != 0 and !soundFinished) {
            if (assets.GetSound(playingSound.ID, generationID)) |loadedSound| {
                var nextSoundInChain = h.Asset.GetNextSoundInChain(assets, playingSound.ID);
                h.Asset.PrefetchSound(assets, nextSoundInChain);
                ...
                for (0..chunksToMix) |loopIndex| {
                    ...
                    if (!IGNORE) {
                        ...
                        const sampleIndex = h.Intrinsics.z._mm_cvttps_epi32(samplePos);
                        const frac: f32x4 = samplePos - h.Intrinsics.z._mm_cvtepi32_ps(sampleIndex);
                ...
                h.Asset.LoadSound(assets, playingSound.ID);
                break;
            }
        }
...

    {
        ...
        for (0..chunkCount) |sampleIndex| {
            const l = h.Intrinsics.i._mm_cvtps_epi32(source0[sampleIndex]);
            const r = h.Intrinsics.i._mm_cvtps_epi32(source1[sampleIndex]);

            const lr1 = h.Intrinsics.z._mm_unpackhi_epi32(l, r);
            const lr0 = h.Intrinsics.z._mm_unpacklo_epi32(l, r);

            const s01 = h.Intrinsics.z._mm_packs_epi32(lr0, lr1);
...
pub fn InitializeAudioState(audioState: *audio_state, arena: *h.Data.memory_arena) void {
