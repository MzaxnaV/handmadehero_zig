...
const i16x8 = h.intrinsics_ns.i16x8;
...
pub const playing_sound = struct {
    ...
    ID: h.file_formats_ns.sound_id,
    samplesPlayed: f32,
    next: ?*playing_sound,
};

pub const audio_state = struct {
    permArena: *h.data_ns.memory_arena,
...
pub fn PlaySound(audioState: *audio_state, soundID: h.file_formats_ns.sound_id) *playing_sound {
...
pub fn OutputPlayingSounds(audioState: *audio_state, soundBuffer: *platform.sound_output_buffer, assets: *h.asset_ns.game_assets, tempArena: *h.data_ns.memory_arena) void {
    debug.TIMED_FUNCTION(.{});
    // AUTOGENERATED ----------------------------------------------------------
    ...
    const mixerMemory = h.data_ns.BeginTemporaryMemory(tempArena);
    defer h.data_ns.EndTemporaryMemory(mixerMemory);

    const generationID = h.asset_ns.BeginGeneration(assets);
    defer h.asset_ns.EndGeneration(assets, generationID);

    assert((soundBuffer.sampleCount & 3) == 0);
    const chunkCount: u32 = (soundBuffer.sampleCount) / 4;

    const realChannel0: []f32x4 = tempArena.PushSlice(f32x4, chunkCount);
    const realChannel1: []f32x4 = tempArena.PushSlice(f32x4, chunkCount);
    ...
    while (playingSoundPtr.*) |playingSound| {
        ...
        while (totalChunksToMix != 0 and !soundFinished) {
            if (assets.GetSound(playingSound.ID, generationID)) |loadedSound| {
                var nextSoundInChain = h.asset_ns.GetNextSoundInChain(assets, playingSound.ID);
                h.asset_ns.PrefetchSound(assets, nextSoundInChain);
                ...
                // channel 0
                const masterVolume0: f32x4 = @splat(audioState.masterVolume[0]);
                var volume0: f32x4 = .{
                ...
                const dVolume0: f32x4 = @splat(dVolume[0]);
                _ = dVolume0;
                const dVolumeChunk0: f32x4 = @splat(dVolumeChunk[0]);

                // channel 1
                const masterVolume1: f32x4 = @splat(audioState.masterVolume[1]);
                var volume1: f32x4 = .{
                ...
                const dVolume1: f32x4 = @splat(dVolume[1]);
                _ = dVolume1;
                const dVolumeChunk1: f32x4 = @splat(dVolumeChunk[1]);

                assert(playingSound.samplesPlayed >= 0);

                var chunksToMix = totalChunksToMix;
                const realChunksRemainingInSound: f32 = @as(f32, @floatFromInt(loadedSound.sampleCount - RoundF32ToInt(u32, playingSound.samplesPlayed))) / dSampleChunk;
                const chunksRemainingInSound = RoundF32ToInt(u32, realChunksRemainingInSound);
                ...
                for (0..chunksToMix) |loopIndex| {
                    ...
                    var sampleValue: f32x4 = .{ 0, 0, 0, 0 };

                    if (!ignore) {
                        const samplePos = f32x4{
                        ...
                        const sampleIndex = h.intrinsics_ns.z._mm_cvttps_epi32(samplePos);
                        const frac: f32x4 = samplePos - h.intrinsics_ns.z._mm_cvtepi32_ps(sampleIndex);

                        const sampleValueF = f32x4{
                            @floatFromInt(loadedSound.samples[0].?[@intCast(sampleIndex[0])]),
                            @floatFromInt(loadedSound.samples[0].?[@intCast(sampleIndex[0])]),
                            @floatFromInt(loadedSound.samples[0].?[@intCast(sampleIndex[0])]),
                            @floatFromInt(loadedSound.samples[0].?[@intCast(sampleIndex[0])]),
                        };
                        const sampleValueC = f32x4{
                        ...
                        sampleValue = f32x4{
                            @floatFromInt(loadedSound.samples[0].?[RoundF32ToInt(u32, samplePosition + 0 * dSample)]),
                            @floatFromInt(loadedSound.samples[0].?[RoundF32ToInt(u32, samplePosition + 1 * dSample)]),
                            @floatFromInt(loadedSound.samples[0].?[RoundF32ToInt(u32, samplePosition + 2 * dSample)]),
                            @floatFromInt(loadedSound.samples[0].?[RoundF32ToInt(u32, samplePosition + 3 * dSample)]),
                        };
                    }
                ...
                h.asset_ns.LoadSound(assets, playingSound.ID);
                break;
            }
        }
...

    {
        ...
        var sampleOut: [*]i16x8 = @ptrCast(soundBuffer.samples);
        for (0..chunkCount) |sampleIndex| {
            const l = h.intrinsics_ns.i._mm_cvtps_epi32(source0[sampleIndex]);
            const r = h.intrinsics_ns.i._mm_cvtps_epi32(source1[sampleIndex]);

            const lr1 = h.intrinsics_ns.z._mm_unpackhi_epi32(l, r);
            const lr0 = h.intrinsics_ns.z._mm_unpacklo_epi32(l, r);

            const s01 = h.intrinsics_ns.z._mm_packs_epi32(lr0, lr1);
...
pub fn InitializeAudioState(audioState: *audio_state, arena: *h.data_ns.memory_arena) void {
