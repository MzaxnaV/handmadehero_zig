...
pub fn FillGroundChunkWork(_: ?*platform.work_queue, data: *anyopaque) void {
    ...
    var __t_blk__0 = debug.TIMED_FUNCTION__impl(0, @src()).Init(.{});
    defer __t_blk__0.End();
...
pub export fn UpdateAndRender(
    ...
    debug.TIMED_FUNCTION(.{});
    var __t_blk__1 = debug.TIMED_FUNCTION__impl(1, @src()).Init(.{});
    defer __t_blk__1.End();
    ...
    if (!tranState.initialized) {
        ...
        debug.renderGroup = h.render_group.Allocate(tranState.assets, &tranState.tranArena, platform.MegaBytes(16), false);
    ...
    if (debug.renderGroup) |debugRenderGroup| {
        h.BeginRender(debugRenderGroup);
        debug.DEBUGReset(tranState.assets, buffer.width, buffer.height);
    }
    ...
    if (debug.renderGroup) |debugRenderGroup| {
        debug.TIMED_BLOCK("DEBUGRenderGroup", .{});

        var __t_blk__2 = debug.TIMED_BLOCK__impl("DEBUGRenderGroup", 2, @src()).Init(.{});
