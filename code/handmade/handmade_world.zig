...
pub fn GetWorldChunk(memoryArena: ?*h.Data.memory_arena, w: *world, chunkX: i32, chunkY: i32, chunkZ: i32) ?*world_chunk {
...
inline fn RecanonicalizeCoord(chunkDim: f32, tile: *i32, tileRel: *f32) void {
    const offSet = h.Intrinsics.RoundF32ToInt(i32, tileRel.* / chunkDim);
    tile.* +%= offSet;
...
pub inline fn Substract(w: *const world, a: *const world_position, b: *const world_position) h.v3 {
    ...
    const result = h.Add(h.Math.Hammard(w.chunkDimInMeters, dTile), h.Sub(a.offset_, b.offset_));
...
pub fn ChangeEntityLocationRaw(arena: *h.Data.memory_arena, w: *world, lowEntityIndex: u32, oldP: ?*const world_position, newP: ?*const world_position) void {
    Debug.TIMED_FUNCTION(.{});
    // AUTOGENERATED ----------------------------------------------------------
...
pub fn ChangeEntityLocation(arena: *h.Data.memory_arena, w: *world, lowEntityIndex: u32, lowEntity: *h.Data.low_entity, newPInit: world_position) void {
    Debug.TIMED_FUNCTION(.{});
    // AUTOGENERATED ----------------------------------------------------------
    ...
    if (!h.Entity.IsSet(&lowEntity.sim, @intFromEnum(h.SimRegion.sim_entity_flags.NonSpatial)) and IsValid(lowEntity.p)) {
        oldP = &lowEntity.p;
    }
    ...
    if (newP) |p| {
        lowEntity.p = p.*;
        h.Entity.ClearFlags(&lowEntity.sim, @intFromEnum(h.SimRegion.sim_entity_flags.NonSpatial));
    } else {
        lowEntity.p = NullPosition();
        h.Entity.AddFlags(&lowEntity.sim, @intFromEnum(h.SimRegion.sim_entity_flags.NonSpatial));
    }
}
