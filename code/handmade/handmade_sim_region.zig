...
const IsSet = h.entity_ns.IsSet;
const MapIntoChunkSpace = h.world_ns.MapIntoChunkSpace;
...
pub const sim_region = struct {
    ...
    origin: h.world_ns.world_position,
    bounds: h.rect3,
...
pub inline fn LoadEntityReference(gameState: *game_state, simRegion: *sim_region, ref: *entity_reference) void {
    switch (ref.*) {
        .index => {
            if (ref.index > 0) {
                ...
                if (entry.ptr) |_| {} else {
                    ...
                    const lowEntity = h.data_ns.GetLowEntity(gameState, ref.index);
                    const p = GetSimSpaceP(simRegion, lowEntity.?);
                    entry.ptr = AddEntity(gameState, simRegion, ref.index, h.data_ns.GetLowEntity(gameState, ref.index), &p);
                }
...
fn AddEntityRaw(gameState: *game_state, simRegion: *sim_region, storageIndex: u32, source: ?*h.data_ns.low_entity) ?*sim_entity {
    ...
    if (entry.ptr) |_| {} else {
        if (simRegion.entityCount < simRegion.maxEntityCount) {
            ...
            if (source) |lowEntity| {
                ...
                assert(!IsSet(&lowEntity.sim, @intFromEnum(sim_entity_flags.Simming)));
                h.entity_ns.AddFlags(&lowEntity.sim, @intFromEnum(sim_entity_flags.Simming));
            }
...
pub fn AddEntity(gameState: *game_state, simRegion: *sim_region, storageIndex: u32, source: ?*h.data_ns.low_entity, simP: ?*const h.v3) ?*sim_entity {
...
pub inline fn GetSimSpaceP(simRegion: *sim_region, stored: *h.data_ns.low_entity) h.v3 {
    var result = h.entity_ns.Invalid;
    if (!IsSet(&stored.sim, @intFromEnum(sim_entity_flags.NonSpatial))) {
        result = h.world_ns.Substract(simRegion.world, &stored.p, &simRegion.origin);
    }

    return result;
}

pub fn BeginSim(simArena: *h.data_ns.memory_arena, gameState: *game_state, world: *h.world_ns.world, origin: h.world_ns.world_position, bounds: h.rect3, dt: f32) *sim_region {
    debug.TIMED_FUNCTION(.{});
    // AUTOGENERATED ----------------------------------------------------------
    ...
    h.data_ns.ZeroStruct([4096]sim_entity_hash, &simRegion.hash);
    ...
    const minChunkP = MapIntoChunkSpace(world, simRegion.origin, simRegion.bounds.GetMinCorner());
    const maxChunkP = MapIntoChunkSpace(world, simRegion.origin, simRegion.bounds.GetMaxCorner());
    ...
    while (chunkZ <= maxChunkP.chunkZ) : (chunkZ += 1) {
        ...
        while (chunkY <= maxChunkP.chunkY) : (chunkY += 1) {
            ...
            while (chunkX <= maxChunkP.chunkX) : (chunkX += 1) {
                if (h.world_ns.GetWorldChunk(null, world, chunkX, chunkY, chunkZ)) |chunk| {
                    var block: ?*h.world_ns.world_entity_block = &chunk.firstBlock;
                    while (block) |b| : (block = b.next) {
                        ...
                        while (entityIndexIndex < b.entityCount) : (entityIndexIndex += 1) {
                            ...
                            if (!IsSet(&low.sim, @intFromEnum(sim_entity_flags.NonSpatial))) {
...
pub fn EndSim(region: *sim_region, gameState: *game_state) void {
    debug.TIMED_FUNCTION(.{});
    // AUTOGENERATED ----------------------------------------------------------
    ...
    while (entityIndex < region.entityCount) : (entityIndex += 1) {
        ...
        assert(IsSet(&stored.sim, @intFromEnum(sim_entity_flags.Simming)));
        stored.sim = entity.*;
        assert(!IsSet(&stored.sim, @intFromEnum(sim_entity_flags.Simming)));

        StoreEntityReference(&stored.sim.sword);

        const newP = if (IsSet(entity, @intFromEnum(sim_entity_flags.NonSpatial)))
            h.world_ns.NullPosition()
        else
            MapIntoChunkSpace(gameState.world, region.origin, entity.p);
        h.world_ns.ChangeEntityLocation(&gameState.worldArena, gameState.world, entity.storageIndex, stored, newP);
        ...
        if (entity.storageIndex == gameState.cameraFollowingEntityIndex) {
            ...
            if (config.DEBUGUI_UseRoomBasedCamera) {
                if (h.X(entity.p) > (9)) {
                    newCameraP = MapIntoChunkSpace(gameState.world, newCameraP, .{ 18, 0, 0 });
                }
                if (h.X(entity.p) < -(9)) {
                    newCameraP = MapIntoChunkSpace(gameState.world, newCameraP, .{ -18, 0, 0 });
                }
                if (h.Y(entity.p) > (5)) {
                    newCameraP = MapIntoChunkSpace(gameState.world, newCameraP, .{ 0, 10, 0 });
                }
                if (h.Y(entity.p) < -(5)) {
                    newCameraP = MapIntoChunkSpace(gameState.world, newCameraP, .{ 0, -10, 0 });
                }
...
fn CanCollide(gameState: *game_state, unsortedA: *sim_entity, unsortedB: *sim_entity) bool {
    var result = false;
    ...
    if (a != b) {
        ...
        if (IsSet(a, @intFromEnum(sim_entity_flags.Collides)) and
            IsSet(b, @intFromEnum(sim_entity_flags.Collides)))
        {
            if (!IsSet(a, @intFromEnum(sim_entity_flags.NonSpatial)) and
                !IsSet(b, @intFromEnum(sim_entity_flags.NonSpatial)))
            {
                result = true;
            }

            const hashBucket = a.storageIndex & (gameState.collisionRuleHash.len - 1);
            var rule: ?*h.data_ns.pairwise_collision_rule = gameState.collisionRuleHash[hashBucket];
...
fn HandleCollision(gameState: *game_state, unsortedA: *sim_entity, unsortedB: *sim_entity) bool {
    var stopsOnCollision = false;

    if (unsortedA.entityType == .Sword) {
        h.data_ns.AddCollisionRule(gameState, unsortedA.storageIndex, unsortedB.storageIndex, false);
...
pub fn CanOverlap(_: *game_state, mover: *sim_entity, region: *sim_entity) bool {
    var result = false;
...
pub fn HandleOverlap(_: *game_state, mover: *sim_entity, region: *sim_entity, _: f32, ground: *f32) void {
    if (region.entityType == .Stairwell) {
        ground.* = h.entity_ns.GetStairGround(region, h.entity_ns.GetEntityGroundPoint(mover));
    }
}
...
pub fn SpeculativeCollide(mover: *sim_entity, region: *sim_entity, testP: h.v3) bool {
    ...
    if (region.entityType == .Stairwell) {
        ...
        const moverGroundPoint = h.entity_ns.GetEntityGroundPointForEntityP(mover, testP);
        const ground = h.entity_ns.GetStairGround(region, moverGroundPoint);

        // ignore
        // result = (h.intrinsics_ns.AbsoluteValue(h.Z(h.entity_ns.GetEntityGroundPoint(mover)) - ground) > stepHeight) or ((h.Y(bary) > 0.1) and (h.Y(bary) < 0.9));
        result = (h.intrinsics_ns.AbsoluteValue(h.Z(h.entity_ns.GetEntityGroundPoint(mover)) - ground) > stepHeight);
    }
...
pub fn EntitiesOverlap(entity: *sim_entity, testEntity: *sim_entity, epsilon: h.v3) bool {
    ...
    while (!result and (volumeIndex < entity.collision.volumes.len)) : (volumeIndex += 1) {
        ...
        while (!result and testVolumeIndex < (testEntity.collision.volumes.len)) : (testVolumeIndex += 1) {
            ...
            result = h.math_ns.RectanglesIntersect(entityRect, testEntityRect);
        }
    }
...
pub fn MoveEntity(gameState: *game_state, simRegion: *sim_region, entity: *sim_entity, dt: f32, moveSpec: *const move_spec, accelaration: h.v3) void {
    debug.TIMED_FUNCTION(.{});
    // AUTOGENERATED ----------------------------------------------------------
    ...
    assert(!IsSet(entity, @intFromEnum(sim_entity_flags.NonSpatial)));

    var ddP = accelaration;
    // const world = gameState.world;

    if (moveSpec.unitMaxAccelVector) {
        const ddPLength = h.math_ns.LengthSq(ddP);
        if (ddPLength > 1.0) {
            ddP = h.Scale(ddP, 1.0 / h.intrinsics_ns.SquareRoot(ddPLength));
        }
    }

    ddP = h.Scale(ddP, moveSpec.speed);
    h.AddTo(&ddP, h.Scale(entity.dP, -moveSpec.drag));
    if (!IsSet(entity, @intFromEnum(sim_entity_flags.ZSupported))) {
        h.AddTo(&ddP, h.v3{ 0, 0, -9.8 });
    }

    // NOTE (Manav): playerDelta = (0.5 * ddP * square(dt)) + entity.dP * dt;
    var playerDelta = h.Add((h.Scale(ddP, 0.5 * h.math_ns.Square(dt))), h.Scale(entity.dP, dt));
    h.AddTo(&entity.dP, h.Scale(ddP, dt));
    assert(h.math_ns.LengthSq(entity.dP) <= h.math_ns.Square(simRegion.maxEntityVelocity));
    ...
    while (iteration < 4) : (iteration += 1) {
        var tMin = @as(f32, 1.0);
        var tMax = @as(f32, 0.0);
        const playerDeltaLength = h.math_ns.Length(playerDelta);
        if (playerDeltaLength > 0) {
            ...
            if (!IsSet(entity, @intFromEnum(sim_entity_flags.NonSpatial))) {
                ...
                while (testHighEntityIndex < simRegion.entityCount) : (testHighEntityIndex += 1) {
                    ...
                    if (((IsSet(testEntity, @intFromEnum(sim_entity_flags.Traversable)) and
                        ...
                        while (volumeIndex < entity.collision.volumes.len) : (volumeIndex += 1) {
                            ...
                            while (testVolumeIndex < testEntity.collision.volumes.len) : (testVolumeIndex += 1) {
                                ...
                                if ((h.Z(rel) >= h.Z(minCorner)) and (h.Z(rel) < h.Z(maxCorner))) {
                                    ...
                                    if (IsSet(testEntity, @intFromEnum(sim_entity_flags.Traversable))) {
                                        var tMaxTest = tMax;
                                        var hitThis = false;
            ...
            if (hitEntity) |_| {
                ...
                if (stopsOnCollision) {
                    ...
                    h.SubFrom(&playerDelta, h.Scale(wallNormal, 1 * h.math_ns.Inner(playerDelta, wallNormal)));
                    // NOTE (Manav): entity.dP -= (1 * Inner(entity.dP, wallNormal))*wallNormal;
                    h.SubFrom(&entity.dP, h.Scale(wallNormal, 1 * h.math_ns.Inner(entity.dP, wallNormal)));
                }
            } else {
                break;
            }
    ...
    ground += h.Z(entity.p) - h.Z(h.entity_ns.GetEntityGroundPoint(entity));
    if ((h.Z(entity.p) <= ground) or
        (IsSet(entity, @intFromEnum(sim_entity_flags.ZSupported)) and
            (h.Z(entity.dP) == 0)))
    {
        entity.p[2] = ground;
        entity.dP[2] = 0;
        h.entity_ns.AddFlags(entity, @intFromEnum(sim_entity_flags.ZSupported));
    } else {
        h.entity_ns.ClearFlags(entity, @intFromEnum(sim_entity_flags.ZSupported));
    }
