...
const RoundF32ToInt = h.intrinsics_ns.RoundF32ToInt;

const rect2i = h.math_ns.rect2i;
const f32x4 = h.intrinsics_ns.f32x4;
const i32x4 = h.intrinsics_ns.i32x4;
const u32x4 = h.intrinsics_ns.u32x4;
...
pub const loaded_bitmap = extern struct {
    ...
    pub fn DrawRectangle(buffer: *const loaded_bitmap, vMin: h.v2, vMax: h.v2, colour: h.v4, clipRect: rect2i, even: bool) void {
        ...
        var fillRect = rect2i{
            .xMin = RoundF32ToInt(i32, h.X(vMin)),
            .yMin = RoundF32ToInt(i32, h.Y(vMin)),
            .xMax = RoundF32ToInt(i32, h.X(vMax)),
            .yMax = RoundF32ToInt(i32, h.Y(vMax)),
        };
        ...
        // zig fmt: off
        const colour32: u32 = (RoundF32ToInt(u32, a * 255.0) << 24) | 
                            (RoundF32ToInt(u32, r * 255.0) << 16) | 
                            (RoundF32ToInt(u32, g * 255.0) << 8) | 
                            (RoundF32ToInt(u32, b * 255) << 0);
        // zig fmt: on
    ...
    // zig fmt: on
    {
        ...
        const xAxisLength = h.math_ns.Length(xAxis);
        const yAxisLength = h.math_ns.Length(yAxis);
        ...
        const invXAxisLengthSq = 1 / h.math_ns.LengthSq(xAxis);
        const invYAxisLengthSq = 1 / h.math_ns.LengthSq(yAxis);

        // zig fmt: off
        const colour32: u32 = (RoundF32ToInt(u32, h.A(colour) * 255.0) << 24) | 
                            (RoundF32ToInt(u32, h.R(colour) * 255.0) << 16) | 
                            (RoundF32ToInt(u32, h.G(colour) * 255.0) << 8) | 
                            (RoundF32ToInt(u32, h.B(colour) * 255.0) << 0);
        // zig fmt: on
        ...
        for (p) |testP| {
            const floorX = h.intrinsics_ns.FloorF32ToI32(h.X(testP));
            const ceilX = h.intrinsics_ns.CeilF32ToI32(h.X(testP));
            const floorY = h.intrinsics_ns.FloorF32ToI32(h.Y(testP));
            const ceilY = h.intrinsics_ns.CeilF32ToI32(h.Y(testP));
        ...
        // AUTOGENERATED ----------------------------------------------------------
        var __t_blk__22 = debug.TIMED_BLOCK__impl(22, @src()).Init("PixelFIll", .{ .hitCount = @intCast((xMax - xMin + 1) * (yMax - yMin + 1)) });
                    ...
                    if (edge0 < 0 and edge1 < 0 and edge2 < 0 and edge3 < 0) {
                        ...
                        if (normalMapOptional) |normalMap| {
                            ...
                            if (ignore) {
                                ...
                                texel = h.math_ns.Hammard(texel, .{ h.A(texel), h.A(texel), h.A(texel), 1 });
                        ...
                        texel = h.math_ns.Hammard(texel, colour);
    ...
    pub fn DrawRectangleQuickly(buffer: *const loaded_bitmap, origin: h.v2, xAxis: h.v2, yAxis: h.v2, notPremultipliedColour: h.v4, 
                                texture: *const loaded_bitmap, pixelsToMeters: f32, clipRect: rect2i, even: bool) void
    // zig fmt: on
    {
        debug.TIMED_FUNCTION(.{});
        // AUTOGENERATED ----------------------------------------------------------
        ...
        const invXAxisLengthSq: f32 = 1.0 / h.math_ns.LengthSq(xAxis);
        const invYAxisLengthSq: f32 = 1.0 / h.math_ns.LengthSq(yAxis);

        var fillRect = rect2i.InvertedInfinityRectangle();

        const p: [4]h.v2 = .{ origin, h.Add(origin, xAxis), h.Add(origin, h.Add(xAxis, yAxis)), h.Add(origin, yAxis) };
        for (p) |testP| {
            const floorX = h.intrinsics_ns.FloorF32ToI32(h.X(testP));
            const ceilX = h.intrinsics_ns.CeilF32ToI32(h.X(testP)) + 1;
            const floorY = h.intrinsics_ns.FloorF32ToI32(h.Y(testP));
            const ceilY = h.intrinsics_ns.CeilF32ToI32(h.Y(testP)) + 1;
        ...
        // const clipRect = rect2i{ .xMin = 0, .yMin = 0, .xMax = widthMax, .yMax = heightMax };
        // const clipRect = rect2i{ .xMin = 128, .yMin = 128, .xMax = 256, .yMax = 256 };
        fillRect.Intersect(clipRect);
        if (!even == (fillRect.yMin & 1 != 0)) {
            fillRect.yMin += 1;
        }

        const u1x4 = h.intrinsics_ns.u1x4;

        if (fillRect.HasArea()) {
            var startClipMask: u1x4 = .{ 0x1, 0x1, 0x1, 0x1 };
            var endClipMask: u1x4 = .{ 0x1, 0x1, 0x1, 0x1 };

            const StartClipMasks: [4]u1x4 = [_]u1x4{
                @bitCast(@as(u4, @bitCast(u1x4{ 0x1, 0x1, 0x1, 0x1 })) << 0), // .{ 0x1, 0x1, 0x1, 0x1 },
                @bitCast(@as(u4, @bitCast(u1x4{ 0x1, 0x1, 0x1, 0x1 })) << 1), // .{ 0x0, 0x1, 0x1, 0x1 },
                @bitCast(@as(u4, @bitCast(u1x4{ 0x1, 0x1, 0x1, 0x1 })) << 2), // .{ 0x0, 0x0, 0x1, 0x1 },
                @bitCast(@as(u4, @bitCast(u1x4{ 0x1, 0x1, 0x1, 0x1 })) << 3), // .{ 0x0, 0x0, 0x0, 0x1 },
            };

            const EndClipMasks: [4]u1x4 = [_]u1x4{
                @bitCast(@as(u4, @bitCast(u1x4{ 0x1, 0x1, 0x1, 0x1 })) >> 0), // .{ 0x1, 0x1, 0x1, 0x1 },
                @bitCast(@as(u4, @bitCast(u1x4{ 0x1, 0x1, 0x1, 0x1 })) >> 3), // .{ 0x1, 0x0, 0x0, 0x0 },
                @bitCast(@as(u4, @bitCast(u1x4{ 0x1, 0x1, 0x1, 0x1 })) >> 2), // .{ 0x1, 0x1, 0x0, 0x0 },
                @bitCast(@as(u4, @bitCast(u1x4{ 0x1, 0x1, 0x1, 0x1 })) >> 1), // .{ 0x1, 0x1, 0x1, 0x0 },
            };
            ...
            const inv255_4x: f32x4 = @splat(inv255);
            // const one255 = 255;
            // const one255_4x = @splat(4, @as(f32, one255));

            // const normalizedC = 1.0 / 255.0;
            // const normalizedSqC = 1.0 / h.math_ns.Square(255.0);

            const one: f32x4 = @splat(1);
            const half: f32x4 = @splat(0.5);
            const four_4x: f32x4 = @splat(4);
            const zero: f32x4 = @splat(0);
            const maskff: i32x4 = @splat(0xff);
            const maskffff: i32x4 = @splat(0xffff);
            const maskff00ff: i32x4 = @splat(0xff00ff);

            const colourr_4x: f32x4 = @splat(h.R(colour));
            const colourg_4x: f32x4 = @splat(h.G(colour));
            const colourb_4x: f32x4 = @splat(h.B(colour));
            const coloura_4x: f32x4 = @splat(h.A(colour));

            const nXAxisx_4x: f32x4 = @splat(h.X(nXAxis));
            const nXAxisy_4x: f32x4 = @splat(h.Y(nXAxis));

            const nYAxisx_4x: f32x4 = @splat(h.X(nYAxis));
            const nYAxisy_4x: f32x4 = @splat(h.Y(nYAxis));

            const originx_4x: f32x4 = @splat(h.X(origin));
            const originy_4x: f32x4 = @splat(h.Y(origin));
            const maxColourValue: f32x4 = @splat(255 * 255);

            const texturePitch_4x: i32x4 = @splat(texture.pitch);

            const widthM2: f32x4 = @splat(@as(f32, @floatFromInt(texture.width - 2)));
            const heightM2: f32x4 = @splat(@as(f32, @floatFromInt(texture.height - 2)));
            ...
            // AUTOGENERATED ----------------------------------------------------------
            var __t_blk__24 = debug.TIMED_BLOCK__impl(24, @src()).Init("PixelFill", .{ .hitCount = @intCast(@divFloor(fillRect.GetClampedRectArea(), 5)) });
            ...
            while (y < yMax) : (y += 2) {
                ...
                const pixelPY = @as(f32x4, @splat(@as(f32, @floatFromInt(y)))) - originy_4x;
                ...
                var clipMask: u1x4 = startClipMask;

                var xi = xMin;
                while (xi < xMax) : (xi += 4) {
                    var u: f32x4 = pixelPX * nXAxisx_4x + pynX;
                    var v: f32x4 = pixelPX * nYAxisx_4x + pynY;

                    var writeMask: u1x4 = @as(u1x4, @bitCast((u >= zero))) & @as(u1x4, @bitCast((u <= one))) &
                        @as(u1x4, @bitCast((v >= zero))) & @as(u1x4, @bitCast((v <= one)));

                    writeMask = writeMask & clipMask;

                    // if (@reduce(.Or, writeMask) != 0)
                    {
                        const originalDest: u32x4 = pixel[0..4].*;

                        u = @min(@max(u, zero), one);
                        v = @min(@max(v, zero), one);

                        const tX: f32x4 = (u * widthM2) + half;
                        const tY: f32x4 = (v * heightM2) + half;

                        var fetchX_4x: i32x4 = h.intrinsics_ns.z._mm_cvttps_epi32(tX);
                        var fetchY_4x: i32x4 = h.intrinsics_ns.z._mm_cvttps_epi32(tY);

                        const fX = tX - h.intrinsics_ns.z._mm_cvtepi32_ps(fetchX_4x);
                        const fY = tY - h.intrinsics_ns.z._mm_cvtepi32_ps(fetchY_4x);

                        fetchX_4x = fetchX_4x << @splat(2);
                        fetchY_4x *= texturePitch_4x;

                        // TODO (Manav): Investigate why below doesn't work
                        // fetchX_4x = h.intrinsics_ns.z._mm_mullo_epi16(fetchX_4x, texturePitch_4x) | (h.intrinsics_ns.z._mm_mulhi_epi16(fetchX_4x, texturePitch_4x) << @splat(4, @as(u5, 16)));
                        ...
                        var texelArb: i32x4 = @as(i32x4, @bitCast(sampleA)) & maskff00ff;
                        var texelAag: i32x4 = @as(i32x4, @bitCast(sampleA >> @as(u5x4, @splat(8)))) & maskff00ff;
                        texelArb = h.intrinsics_ns.z._mm_mullo_epi16(texelArb, texelArb);
                        const texelAa: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(texelAag >> @as(u5x4, @splat(16)))) & maskff); // cvtepi32
                        texelAag = h.intrinsics_ns.z._mm_mullo_epi16(texelAag, texelAag);

                        var texelBrb: i32x4 = @as(i32x4, @bitCast(sampleB)) & maskff00ff;
                        var texelBag: i32x4 = @as(i32x4, @bitCast(sampleB >> @as(u5x4, @splat(8)))) & maskff00ff;
                        texelBrb = h.intrinsics_ns.z._mm_mullo_epi16(texelBrb, texelBrb);
                        const texelBa: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(texelBag >> @as(u5x4, @splat(16)))) & maskff);
                        texelBag = h.intrinsics_ns.z._mm_mullo_epi16(texelBag, texelBag);

                        var texelCrb: i32x4 = @as(i32x4, @bitCast(sampleC)) & maskff00ff;
                        var texelCag: i32x4 = @as(i32x4, @bitCast(sampleC >> @as(u5x4, @splat(8)))) & maskff00ff;
                        texelCrb = h.intrinsics_ns.z._mm_mullo_epi16(texelCrb, texelCrb);
                        const texelCa: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(texelCag >> @as(u5x4, @splat(16)))) & maskff);
                        texelCag = h.intrinsics_ns.z._mm_mullo_epi16(texelCag, texelCag);

                        var texelDrb: i32x4 = @as(i32x4, @bitCast(sampleD)) & maskff00ff;
                        var texelDag: i32x4 = @as(i32x4, @bitCast(sampleD >> @as(u5x4, @splat(8)))) & maskff00ff;
                        texelDrb = h.intrinsics_ns.z._mm_mullo_epi16(texelDrb, texelDrb);
                        const texelDa: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(texelDag >> @as(u5x4, @splat(16)))) & maskff);
                        texelDag = h.intrinsics_ns.z._mm_mullo_epi16(texelDag, texelDag);

                        // var texelAb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleA) & maskff);
                        // var texelAg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleA >> @splat(4, @as(u5, 8))) & maskff);
                        // var texelAr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleA >> @splat(4, @as(u5, 16))) & maskff);
                        // var texelAa: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleA >> @splat(4, @as(u5, 24))) & maskff);

                        // var texelBb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleB) & maskff);
                        // var texelBg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleB >> @splat(4, @as(u5, 8))) & maskff);
                        // var texelBr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleB >> @splat(4, @as(u5, 16))) & maskff);
                        // var texelBa: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleB >> @splat(4, @as(u5, 24))) & maskff);

                        // var texelCb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleC) & maskff);
                        // var texelCg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleC >> @splat(4, @as(u5, 8))) & maskff);
                        // var texelCr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleC >> @splat(4, @as(u5, 16))) & maskff);
                        // var texelCa: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleC >> @splat(4, @as(u5, 24))) & maskff);

                        // var texelDb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleD) & maskff);
                        // var texelDg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleD >> @splat(4, @as(u5, 8))) & maskff);
                        // var texelDr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleD >> @splat(4, @as(u5, 16))) & maskff);
                        // var texelDa: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleD >> @splat(4, @as(u5, 24))) & maskff);

                        var destb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(originalDest)) & maskff);
                        var destg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(originalDest >> @as(u5x4, @splat(8)))) & maskff);
                        var destr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(originalDest >> @as(u5x4, @splat(16)))) & maskff);
                        const desta: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(originalDest >> @as(u5x4, @splat(24)))) & maskff);

                        // Notes (Manav): >> operation on i32x4 doesn't generate vpsrld but vpsrad
                        const texelAr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(h.intrinsics_ns.z._mm_srli_epi32(texelArb, 16));
                        const texelAg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(texelAag & maskffff);
                        const texelAb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(texelArb & maskffff);

                        const texelBr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(h.intrinsics_ns.z._mm_srli_epi32(texelBrb, 16));
                        const texelBg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(texelBag & maskffff);
                        const texelBb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(texelBrb & maskffff);

                        const texelCr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(h.intrinsics_ns.z._mm_srli_epi32(texelCrb, 16));
                        const texelCg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(texelCag & maskffff);
                        const texelCb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(texelCrb & maskffff);

                        const texelDr: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(h.intrinsics_ns.z._mm_srli_epi32(texelDrb, 16));
                        const texelDg: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(texelDag & maskffff);
                        const texelDb: f32x4 = h.intrinsics_ns.z._mm_cvtepi32_ps(texelDrb & maskffff);
                        ...
                        const invTexelA: f32x4 = one - (texela * inv255_4x);
                        ...
                        blendedr = blendedr * h.intrinsics_ns.i._mm_rsqrt_ps(blendedr);
                        blendedg = blendedg * h.intrinsics_ns.i._mm_rsqrt_ps(blendedg);
                        blendedb = blendedb * h.intrinsics_ns.i._mm_rsqrt_ps(blendedb);
                        blendeda = blendeda;

                        const intr: i32x4 = h.intrinsics_ns.i._mm_cvtps_epi32(blendedr);
                        const intg: i32x4 = h.intrinsics_ns.i._mm_cvtps_epi32(blendedg);
                        const intb: i32x4 = h.intrinsics_ns.i._mm_cvtps_epi32(blendedb);
                        const inta: i32x4 = h.intrinsics_ns.i._mm_cvtps_epi32(blendeda);

                        const sr: i32x4 = (intr << @as(u5x4, @splat(16)));
                        const sg: i32x4 = (intg << @as(u5x4, @splat(8)));
                        const sb: i32x4 = intb;
                        const sa: i32x4 = (inta << @as(u5x4, @splat(24)));

                        const out: i32x4 = sr | sg | sb | sa;

                        const bx4 = h.intrinsics_ns.bx4;
                        const maskedOut: u32x4 = @select(u32, @as(bx4, @bitCast(writeMask)), @as(u32x4, @bitCast(out)), originalDest);

                        // @as(*align(1) u32x4, @alignCast(@ptrCast(pixel))).* = maskedOut;
                        @as(*u32x4, @ptrCast(@alignCast(pixel))).* = maskedOut;
                    }
    ...
    pub fn ChangeSaturation(buffer: *const loaded_bitmap, level: f32) void {
        ...
        while (y < buffer.height) : (y += 1) {
            const dest: [*]u32 = @ptrCast(@alignCast(destRow));

            var x = @as(i32, 0);
            while (x < buffer.width) : (x += 1) {
                const index: u32 = @intCast(x);
    ...
    pub fn DrawBitmap(buffer: *const loaded_bitmap, bitmap: *const loaded_bitmap, realX: f32, realY: f32, cAlpha: f32) void {
        ...
        while (y < maxY) : (y += 1) {
            const dest: [*]u32 = @ptrCast(@alignCast(destRow));
            const source: [*]align(1) u32 = @ptrCast(sourceRow);
            var x = minX;
            while (x < maxX) : (x += 1) {
                const index: u32 = @intCast(x - minX);
...
pub const render_group = struct {
    ...
    pub fn Allocate(assets: *h.asset_ns.game_assets, arena: *h.data_ns.memory_arena, maxPushBufferSize: u32, rendersInBackground: bool) *Self {
    ...
    pub fn Perspective(self: *Self, pixelWidth: u32, pixelHeight: u32, metersToPixels: f32, focalLength: f32, distanceAboveTarget: f32) void {
        const pixelsToMeters = h.math_ns.SafeRatiof1(1, metersToPixels);
    ...
    fn PushRenderElements(self: *Self, comptime t: render_group_entry_type) ?*align(1) t.Type() {
        debug.TIMED_FUNCTION(.{});
        // AUTOGENERATED ----------------------------------------------------------
        var __t_blk__29 = debug.TIMED_FUNCTION__impl(29, @src()).Init(.{});
    ...
    pub fn PushBitmap2(self: *Self, ID: h.file_formats_ns.bitmap_id, height: f32, offset: h.v3, colour: h.v4) void {
        ...
        if (self.rendersInBackground and bitmap == null) {
            h.asset_ns.LoadBitmap(self.assets, ID, true);
    ...
    pub fn PushFont(self: *Self, ID: h.file_formats_ns.font_id) ?*h.asset_ns.loaded_font {
        const font = self.assets.GetFont(ID, self.generationID);
        if (font) |f| {
            _ = f;
        } else {
            assert(!self.rendersInBackground);
            h.asset_ns.LoadFont(self.assets, ID, false);
            self.missingResourceCount += 1;
        }
    ...
    fn RenderGroupToOutput(self: *Self, outputTarget: *loaded_bitmap, clipRect: rect2i, even: bool) void {
        debug.TIMED_FUNCTION(.{});
        // AUTOGENERATED ----------------------------------------------------------
        ...
        var baseAddress: u32 = 0;
        while (baseAddress < self.pushBufferSize) {
            const ptr: [*]u8 = self.pushBufferBase + baseAddress;
            const header = @as(*render_group_entry_header, @ptrCast(@alignCast(ptr)));
    ...
    fn DoTiledRenderWork(_: ?*platform.work_queue, data: *anyopaque) void {
        ...
        const work: *tile_render_work = @as(*tile_render_work, @ptrCast(@alignCast(data)));
    ...
    pub fn TiledRenderGroupToOutput(self: *Self, renderQueue: *platform.work_queue, outputTarget: *loaded_bitmap) void {
        ...
        while (tileY < tileCountY) : (tileY += 1) {
            ...
            while (tileX < tileCountX) : (tileX += 1) {
                ...
                if (!ignore) {
                    h.data_ns.platformAPI.AddEntry(renderQueue, DoTiledRenderWork, work);
                } else {
        ...
        h.data_ns.platformAPI.CompleteAllWork(renderQueue);
    }
...
pub fn BeginRender(group: ?*render_group) void {
    ...
    if (group) |renderGroup| {
        ...
        renderGroup.generationID = h.asset_ns.BeginGeneration(renderGroup.assets);
    }
}
...
pub fn EndRender(group: ?*render_group) void {
    ...
    if (group) |renderGroup| {
        ...
        h.asset_ns.EndGeneration(renderGroup.assets, renderGroup.generationID);
...
fn SampleEnvironmentMap(screenSpaceUV: h.v2, sampleDirection: h.v3, roughness: f32, map: *environment_map, distanceFromMapInZ: f32) h.v3 {
    const lodIndex: u32 = @intFromFloat(roughness * @as(f32, @floatFromInt(map.lod.len - 1)) + 0.5);
