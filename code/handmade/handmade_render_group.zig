...
const RoundF32ToInt = h.Intrinsics.RoundF32ToInt;

const rect2i = h.Math.rect2i;
const f32x4 = h.Intrinsics.f32x4;
const i32x4 = h.Intrinsics.i32x4;
const u32x4 = h.Intrinsics.u32x4;
...
pub const loaded_bitmap = extern struct {
    ...
    // zig fmt: on
    {
        ...
        const xAxisLength = h.Math.Length(xAxis);
        const yAxisLength = h.Math.Length(yAxis);
        ...
        const invXAxisLengthSq = 1 / h.Math.LengthSq(xAxis);
        const invYAxisLengthSq = 1 / h.Math.LengthSq(yAxis);
        ...
        for (p) |testP| {
            const floorX = h.Intrinsics.FloorF32ToI32(h.X(testP));
            const ceilX = h.Intrinsics.CeilF32ToI32(h.X(testP));
            const floorY = h.Intrinsics.FloorF32ToI32(h.Y(testP));
            const ceilY = h.Intrinsics.CeilF32ToI32(h.Y(testP));
                    ...
                    if (edge0 < 0 and edge1 < 0 and edge2 < 0 and edge3 < 0) {
                        ...
                        if (normalMapOptional) |normalMap| {
                            ...
                            if (IGNORE) {
                                ...
                                texel = h.Math.Hammard(texel, .{ h.A(texel), h.A(texel), h.A(texel), 1 });
                        ...
                        texel = h.Math.Hammard(texel, colour);
    ...
    // zig fmt: on
    {
        ...
        const invXAxisLengthSq: f32 = 1.0 / h.Math.LengthSq(xAxis);
        const invYAxisLengthSq: f32 = 1.0 / h.Math.LengthSq(yAxis);
        ...
        for (p) |testP| {
            const floorX = h.Intrinsics.FloorF32ToI32(h.X(testP));
            const ceilX = h.Intrinsics.CeilF32ToI32(h.X(testP)) + 1;
            const floorY = h.Intrinsics.FloorF32ToI32(h.Y(testP));
            const ceilY = h.Intrinsics.CeilF32ToI32(h.Y(testP)) + 1;
        ...
        if (fillRect.HasArea()) {
            ...
            // const normalizedSqC = 1.0 / h.Math.Square(255.0);
            ...
            while (y < yMax) : (y += 2) {
                ...
                while (xi < xMax) : (xi += 4) {
                    ...
                    // if (@reduce(.Or, writeMask) != 0)
                    {
                        ...
                        var fetchX_4x: i32x4 = h.Intrinsics.z._mm_cvttps_epi32(tX);
                        var fetchY_4x: i32x4 = h.Intrinsics.z._mm_cvttps_epi32(tY);

                        const fX = tX - h.Intrinsics.z._mm_cvtepi32_ps(fetchX_4x);
                        const fY = tY - h.Intrinsics.z._mm_cvtepi32_ps(fetchY_4x);

                        fetchX_4x = fetchX_4x << @splat(2);
                        fetchY_4x *= texturePitch_4x;

                        // TODO (Manav): Investigate why below doesn't work
                        // fetchX_4x = h.Intrinsics.z._mm_mullo_epi16(fetchX_4x, texturePitch_4x) | (h.Intrinsics.z._mm_mulhi_epi16(fetchX_4x, texturePitch_4x) << @splat(4, @as(u5, 16)));
                        ...
                        texelArb = h.Intrinsics.z._mm_mullo_epi16(texelArb, texelArb);
                        const texelAa: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(texelAag >> @as(u5x4, @splat(16)))) & maskff); // cvtepi32
                        texelAag = h.Intrinsics.z._mm_mullo_epi16(texelAag, texelAag);
                        ...
                        texelBrb = h.Intrinsics.z._mm_mullo_epi16(texelBrb, texelBrb);
                        const texelBa: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(texelBag >> @as(u5x4, @splat(16)))) & maskff);
                        texelBag = h.Intrinsics.z._mm_mullo_epi16(texelBag, texelBag);
                        ...
                        texelCrb = h.Intrinsics.z._mm_mullo_epi16(texelCrb, texelCrb);
                        const texelCa: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(texelCag >> @as(u5x4, @splat(16)))) & maskff);
                        texelCag = h.Intrinsics.z._mm_mullo_epi16(texelCag, texelCag);
                        ...
                        texelDrb = h.Intrinsics.z._mm_mullo_epi16(texelDrb, texelDrb);
                        const texelDa: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(texelDag >> @as(u5x4, @splat(16)))) & maskff);
                        texelDag = h.Intrinsics.z._mm_mullo_epi16(texelDag, texelDag);

                        // var texelAb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleA) & maskff);
                        // var texelAg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleA >> @splat(4, @as(u5, 8))) & maskff);
                        // var texelAr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleA >> @splat(4, @as(u5, 16))) & maskff);
                        // var texelAa: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleA >> @splat(4, @as(u5, 24))) & maskff);

                        // var texelBb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleB) & maskff);
                        // var texelBg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleB >> @splat(4, @as(u5, 8))) & maskff);
                        // var texelBr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleB >> @splat(4, @as(u5, 16))) & maskff);
                        // var texelBa: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleB >> @splat(4, @as(u5, 24))) & maskff);

                        // var texelCb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleC) & maskff);
                        // var texelCg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleC >> @splat(4, @as(u5, 8))) & maskff);
                        // var texelCr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleC >> @splat(4, @as(u5, 16))) & maskff);
                        // var texelCa: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleC >> @splat(4, @as(u5, 24))) & maskff);

                        // var texelDb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleD) & maskff);
                        // var texelDg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleD >> @splat(4, @as(u5, 8))) & maskff);
                        // var texelDr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleD >> @splat(4, @as(u5, 16))) & maskff);
                        // var texelDa: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@bitCast(i32x4, sampleD >> @splat(4, @as(u5, 24))) & maskff);

                        var destb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(originalDest)) & maskff);
                        var destg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(originalDest >> @as(u5x4, @splat(8)))) & maskff);
                        var destr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(originalDest >> @as(u5x4, @splat(16)))) & maskff);
                        const desta: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(@as(i32x4, @bitCast(originalDest >> @as(u5x4, @splat(24)))) & maskff);

                        // Notes (Manav): >> operation on i32x4 doesn't generate vpsrld but vpsrad
                        const texelAr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(h.Intrinsics.z._mm_srli_epi32(texelArb, 16));
                        const texelAg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(texelAag & maskffff);
                        const texelAb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(texelArb & maskffff);

                        const texelBr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(h.Intrinsics.z._mm_srli_epi32(texelBrb, 16));
                        const texelBg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(texelBag & maskffff);
                        const texelBb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(texelBrb & maskffff);

                        const texelCr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(h.Intrinsics.z._mm_srli_epi32(texelCrb, 16));
                        const texelCg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(texelCag & maskffff);
                        const texelCb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(texelCrb & maskffff);

                        const texelDr: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(h.Intrinsics.z._mm_srli_epi32(texelDrb, 16));
                        const texelDg: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(texelDag & maskffff);
                        const texelDb: f32x4 = h.Intrinsics.z._mm_cvtepi32_ps(texelDrb & maskffff);
                        ...
                        blendedr = blendedr * h.Intrinsics.i._mm_rsqrt_ps(blendedr);
                        blendedg = blendedg * h.Intrinsics.i._mm_rsqrt_ps(blendedg);
                        blendedb = blendedb * h.Intrinsics.i._mm_rsqrt_ps(blendedb);
                        blendeda = blendeda;

                        const intr: i32x4 = h.Intrinsics.i._mm_cvtps_epi32(blendedr);
                        const intg: i32x4 = h.Intrinsics.i._mm_cvtps_epi32(blendedg);
                        const intb: i32x4 = h.Intrinsics.i._mm_cvtps_epi32(blendedb);
                        const inta: i32x4 = h.Intrinsics.i._mm_cvtps_epi32(blendeda);
...
pub const render_group = struct {
    ...
    pub fn Allocate(assets: *h.Asset.game_assets, arena: *h.Data.memory_arena, maxPushBufferSize: u32, rendersInBackground: bool) *Self {
    ...
    pub fn Perspective(self: *Self, pixelWidth: u32, pixelHeight: u32, metersToPixels: f32, focalLength: f32, distanceAboveTarget: f32) void {
        const pixelsToMeters = h.Math.SafeRatiof1(1, metersToPixels);
    ...
    pub fn PushBitmap2(self: *Self, ID: h.FileFormats.bitmap_id, height: f32, offset: h.v3, defaults: struct { colour: h.v4 = .{ 1.0, 1.0, 1.0, 1.0 }, cAlign: f32 = 1.0 }) void {
        var bitmap = self.assets.GetBitmap(ID, self.generationID);

        if (self.rendersInBackground and bitmap == null) {
            h.Asset.LoadBitmap(self.assets, ID, true);
        ...
        if (bitmap) |b| {
            ...
            h.Asset.LoadBitmap(self.assets, ID, false);
            self.missingResourceCount += 1;
        }
    }

    pub fn PushFont(self: *Self, ID: h.FileFormats.font_id) ?*h.Asset.loaded_font {
        const font = self.assets.GetFont(ID, self.generationID);
        if (font) |f| {
            _ = f;
        } else {
            Assert(!self.rendersInBackground);
            h.Asset.LoadFont(self.assets, ID, false);
            self.missingResourceCount += 1;
        }
    ...
    pub fn TiledRenderGroupToOutput(self: *Self, renderQueue: *Platform.work_queue, outputTarget: *loaded_bitmap) void {
        ...
        while (tileY < tileCountY) : (tileY += 1) {
            ...
            while (tileX < tileCountX) : (tileX += 1) {
                ...
                if (!IGNORE) {
                    h.Data.platformAPI.AddEntry(renderQueue, DoTiledRenderWork, work);
                } else {
        ...
        h.Data.platformAPI.CompleteAllWork(renderQueue);
    }
...
pub inline fn GetBitmapDim(group: *render_group, bitmap: *loaded_bitmap, height: f32, offset: h.v3, cAlign: f32) used_bitmap_dim {
    ...
    dim.alignment = h.Scale(h.Math.Hammard(bitmap.alignPercentage, dim.size), cAlign);
...
pub fn BeginRender(group: ?*render_group) void {
    ...
    if (group) |renderGroup| {
        ...
        renderGroup.generationID = h.Asset.BeginGeneration(renderGroup.assets);
    }
}
...
pub fn EndRender(group: ?*render_group) void {
    ...
    if (group) |renderGroup| {
        ...
        h.Asset.EndGeneration(renderGroup.assets, renderGroup.generationID);
