...
pub const loaded_bitmap = extern struct {
    ...
    pub fn DrawRectangle(buffer: *const loaded_bitmap, vMin: h.v2, vMax: h.v2, colour: h.v4, clipRect: h.rect2i, even: bool) void {
        ...
        // NOTE (Manav): clamp this because of debug draw
        const r = h.Clampf01(h.R(colour));
        const g = h.G(colour);
        const b = h.B(colour);
        const a = h.A(colour);
    ...
    // zig fmt: on
    {
        ...
        debug.TIMED_BLOCK("PixelFIll", .{ .hitCount = @intCast((xMax - xMin + 1) * (yMax - yMin + 1)) });

        var __t_blk__15 = debug.TIMED_BLOCK__impl("PixelFIll", 15, @src()).Init(.{ .hitCount = @intCast((xMax - xMin + 1) * (yMax - yMin + 1)) });
                ...
                if (!ignore) {
                    ...
                    if (edge0 < 0 and edge1 < 0 and edge2 < 0 and edge3 < 0) {
                        ...
                        if (normalMapOptional) |normalMap| {
                            ...
                            const normalA: h.v4 = Unpack4x8(normalSample.a);
                            const normalB: h.v4 = Unpack4x8(normalSample.b);
                            const normalC: h.v4 = Unpack4x8(normalSample.c);
                            const normalD: h.v4 = Unpack4x8(normalSample.d);
                        ...
                        const blended255 = h.Linear1ToSRGB255(blended);
    ...
    // zig fmt: on
    {
        ...
        if (fillRect.HasArea()) {
            ...
            debug.TIMED_BLOCK("PixelFill", .{ .hitCount = @intCast(@divFloor(fillRect.GetClampedRectArea(), 5)) });

            var __t_blk__17 = debug.TIMED_BLOCK__impl("PixelFill", 17, @src()).Init(.{ .hitCount = @intCast(@divFloor(fillRect.GetClampedRectArea(), 5)) });
    ...
    pub fn DrawMatte(buffer: *const loaded_bitmap, bitmap: *const loaded_bitmap, realX: f32, realY: f32, cAlpha: f32) void {
        ...
        var __t_blk__20 = debug.TIMED_FUNCTION__impl(20, @src()).Init(.{});
        defer __t_blk__20.End();
...
fn GetRenderEntityBasisP(transform: *const render_transform, originalP: h.v3) entity_basis_p_result {
    ...
    var __t_blk__21 = debug.TIMED_FUNCTION__impl(21, @src()).Init(.{});
    defer __t_blk__21.End();
...
pub const render_group = struct {
    ...
    fn PushRenderElements(self: *Self, comptime t: render_group_entry_type) ?*align(1) t.Type() {
        ...
        var __t_blk__22 = debug.TIMED_FUNCTION__impl(22, @src()).Init(.{});
        defer __t_blk__22.End();
        ...
        const size: u32 = @sizeOf(element_type) + @sizeOf(render_group_entry_header);
    ...
    fn RenderGroupToOutput(self: *Self, outputTarget: *loaded_bitmap, clipRect: h.rect2i, even: bool) void {
        ...
        var __t_blk__23 = debug.TIMED_FUNCTION__impl(23, @src()).Init(.{});
        defer __t_blk__23.End();
        ...
        while (baseAddress < self.pushBufferSize) {
            ...
            switch (header.entryType) {
                .Clear => {
                    ...
                    const colour: h.v4 = entry.colour;
    ...
    fn DoTiledRenderWork(_: ?*platform.work_queue, data: *anyopaque) void {
        ...
        debug.TIMED_FUNCTION(.{});
        var __t_blk__24 = debug.TIMED_FUNCTION__impl(24, @src()).Init(.{});
        defer __t_blk__24.End();
    ...
    pub fn NonTiledRenderGroupToOutput(self: *Self, outputTarget: *loaded_bitmap) void {
        debug.TIMED_FUNCTION(.{});
        var __t_blk__25 = debug.TIMED_FUNCTION__impl(25, @src()).Init(.{});
    ...
    pub fn TiledRenderGroupToOutput(self: *Self, renderQueue: *platform.work_queue, outputTarget: *loaded_bitmap) void {
        debug.TIMED_FUNCTION(.{});
        var __t_blk__26 = debug.TIMED_FUNCTION__impl(26, @src()).Init(.{});
...
pub fn BeginRender(group: ?*render_group) void {
    debug.TIMED_FUNCTION(.{});
    var __t_blk__27 = debug.TIMED_FUNCTION__impl(27, @src()).Init(.{});
...
pub fn EndRender(group: ?*render_group) void {
    debug.TIMED_FUNCTION(.{});
    var __t_blk__28 = debug.TIMED_FUNCTION__impl(28, @src()).Init(.{});
    defer __t_blk__28.End();
